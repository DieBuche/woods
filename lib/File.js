...
File.prototype.html = function(param) {
	...
	if (this.type == 'image') {
		...
		var cover = this.page.files[this.name + 'MoviePoster.jpg'];
		return '<video autobuffer controls'
			+ ' width=' + cover.width
			+ ' height=' + cover.height
			+ ' poster="' + this.name + 'MoviePoster.jpg">'
			+ '<source src="' + this.name + '.mp4" type="video/mp4">'
			+ '<source src="' + this.name + '.webm" type="video/webm">'
			+ '</video>';
	} else {
